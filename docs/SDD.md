...
当前全量：`node --test` → **78 项全绿**（各分类之和；新增测试务必保持全绿再提 PR）。

> 测试内的"未配置视觉模型"场景必须把 `LLM_VISION_*` **赋空串**而非 `delete`：
> `src/config.js` 的 `loadEnvFile` 只跳过已存在于 `process.env` 的键，
> `delete` 掉的键会被本机 `.env` 的真实配置重新填上，导致用例产生真实付费调用。
